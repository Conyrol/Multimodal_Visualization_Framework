...
class A2C(nn.Module):
    ...
    def forward(self, global_feature, local_feature, senetence_feature, location_feature, hidden_state):
        ...
        state_feature = self.state_fc(state_feature)            # 512+512+128+1024--1024
        state_feature = F.relu(state_feature)

        hidden_state = self.gru(state_feature, hidden_state) #1024--1024
        value = self.critic_linear(hidden_state)                # 1024--1
        actions = self.actor_linear(hidden_state)               # 1024--7 这里不处理，先返回再softmax

        tIoU = self.tiou_resfc(state_feature)                   # 预测一个IOU 1024--1，监督学习
        location = self.location_resfc(state_feature)           # 预测一个location 1024--2，监督学习
